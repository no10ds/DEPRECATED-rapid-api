...
from api.common.config.aws import AWS_REGION, GLUE_CATALOGUE_DB_NAME, GLUE_CRAWLER_ROLE, GLUE_CONNECTION_NAME, \
    GLUE_QUOTE_CHAR, GLUE_CSV_SERIALISATION_LIBRARY, GLUE_TABLE_PRESENCE_CHECK_RETRY_COUNT, \
...
from api.common.custom_exceptions import CrawlerCreateFailsError, CrawlerStartFailsError, \
    CrawlerDeleteFailsError, GetCrawlerError, CrawlerIsNotReadyError, TableDoesNotExistError, TableNotCreatedError
...
class GlueAdapter:
    ...
    def check_crawler_is_ready(self, domain: str, dataset: str):
        if self._get_crawler_state(domain, dataset) != 'READY':
            raise CrawlerIsNotReadyError(f"Crawler is not ready for domain={domain} and dataset={dataset}")
    ...
    def update_table(self, table_name: str):
        ...
        self.glue_client.update_table(DatabaseName=self.glue_catalogue_db_name, TableInput=updated_definition)
    ...
    def get_table_when_created(self, table_name: str) -> Dict:
        for _ in range(GLUE_TABLE_PRESENCE_CHECK_RETRY_COUNT):
            try:
                ...
                AppLogger.info(f"Waiting {GLUE_TABLE_PRESENCE_CHECK_INTERVAL}s for table [{table_name}] to be created")
                sleep(GLUE_TABLE_PRESENCE_CHECK_INTERVAL)
                continue
        raise TableNotCreatedError(
            f"[{table_name}] was not created after {GLUE_TABLE_PRESENCE_CHECK_RETRY_COUNT * GLUE_TABLE_PRESENCE_CHECK_INTERVAL}s")  # noqa: E501
    ...
    def _get_crawler_state(self, domain: str, dataset: str) -> str:
        try:
            ...
            raise GetCrawlerError(f"Failed to get crawler state domain = {domain} dataset = {dataset}")
    ...
    def _table_configured_correctly(self, table_config: Dict) -> bool:
        try:
            serialisation_lib = table_config["Table"]["StorageDescriptor"]["SerdeInfo"]["SerializationLibrary"]
            quote_char = table_config["Table"]["StorageDescriptor"]["SerdeInfo"]["Parameters"]["quoteChar"]
        except KeyError:
            return False
    ...
    def _get_table(self, table_name: str) -> Dict:
        try:
            table = self.glue_client.get_table(DatabaseName=self.glue_catalogue_db_name, Name=table_name)
            return table
        except ClientError as error:
