...
from api.application.services.authorisation_service import protect_dataset_endpoint, user_logged_in, RAPID_ACCESS_TOKEN, \
    extract_user_groups
from api.common.aws_utilities import get_secret
from api.common.config.auth import COGNITO_USER_LOGIN_APP_CREDENTIALS_SECRETS_NAME, construct_user_auth_url
...
async def request_middleware(request: Request, call_next):
    ...
    AppLogger.info(f"    Request started: {request.method} {query_params} with request id: {request_id}")
...
def login(request: Request):
    ...
    cognito_user_login_client_id = get_secret(COGNITO_USER_LOGIN_APP_CREDENTIALS_SECRETS_NAME)["client_id"]
...
def _build_list_of_datasets_from_write_permissions(scope_prefix, user_groups):
    return [user_group.replace(scope_prefix, "")
