...
def transform_and_validate(schema: Schema, data: pd.DataFrame) -> pd.DataFrame:
    validation_context = ValidationContext(data) \
    ...
    if columns_to_cast:
        for column, column_type in columns_to_cast.items():
            try:
                ...
                error_list.append(f"Failed to convert column [{column}] to type [{column_type}]")
    return df, error_list


def dataset_has_correct_columns(df: pd.DataFrame, schema: Schema) -> Tuple[pd.DataFrame, list[str]]:
    ...
    if not has_expected_columns or len(actual_columns) != len(expected_columns):
        ...
        raise DatasetError(f"Expected columns: {expected_columns}, received: {actual_columns}")

    return df, error_list


def dataset_has_acceptable_null_values(data_frame: pd.DataFrame, schema: Schema) -> Tuple[pd.DataFrame, list[str]]:
    error_list = []
...
def dataset_has_correct_data_types(data_frame: pd.DataFrame, schema: Schema) -> Tuple[pd.DataFrame, list[str]]:
    error_list = []
    for column in schema.columns:
...
def dataset_has_no_illegal_characters_in_partition_columns(
        data_frame: pd.DataFrame, schema: Schema) -> Tuple[pd.DataFrame, list[str]]:
    error_list = []
    for column in schema.get_partition_columns():
        ...
        if not column.data_type == DataTypes.DATE and series.dtype == object:
            any_illegal_characters = any([value is True for value in series.str.contains("/")])
            if any_illegal_characters:
                error_list.append(f"Partition column [{column.name}] has values with illegal characters '/'")

    return data_frame, error_list


def convert_dates_to_ymd(df: pd.DataFrame, schema: Schema) -> Tuple[pd.DataFrame, list[str]]:
    error_list = []
    for column in schema.get_columns_by_type(DataTypes.DATE):
        df[column.name], error = convert_date_column_to_ymd(column.name, df[column.name], column.format)
...
def convert_date_column_to_ymd(column_name: str, series: pd.Series, date_format: str) -> Tuple[pd.Series, str]:
    error_result = None
    try:
