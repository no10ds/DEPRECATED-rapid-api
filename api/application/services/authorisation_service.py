...
from api.common.config.auth import IDENTITY_PROVIDER_TOKEN_URL, COGNITO_JWKS_URL, COGNITO_RESOURCE_SERVER_ID, Action, \
    SensitivityLevel, RAPID_ACCESS_TOKEN
from api.common.custom_exceptions import SchemaNotFoundError, AuthorisationError, UserCredentialsUnavailableError
...
class OAuth2ClientCredentials(OAuth2):
    def __init__(
        ...
        if not allowed_scopes:
            allowed_scopes = {}
        flows = OAuthFlowsModel(clientCredentials={"tokenUrl": token_url, "scopes": allowed_scopes})
        super().__init__(
            flows=flows,
...
def protect_endpoint(security_scopes: SecurityScopes,
...
def protect_dataset_endpoint(security_scopes: SecurityScopes,
                             browser_request: str = Depends(is_browser_request),
                             client_token: str = Depends(oauth2_scheme),
                             user_token: str = Depends(oauth2_user_scheme),
    ...
    if user_token is not None:
        ...
        if browser_request:
            ...
            if client_token:
                check_client_app_permissions(client_token, dataset, domain, security_scopes)
            else:
...
def extract_user_groups(token: str) -> List[str]:
    try:
        ...
        raise AuthorisationError("Not enough permissions or access token is missing/invalid")
...
def match_client_app_permissions(token_scopes: list, endpoint_scopes: list, domain: str = None, dataset: str = None):
...
def match_user_permissions(token_scopes: list, endpoint_scopes: list, domain: Optional[str] = None,
                           dataset: Optional[str] = None):
    if domain and dataset:
        allowed_permissions = [f"{permission}/{domain}/{dataset}" for permission in endpoint_scopes]
        if not any([allowed_permission in token_scopes for allowed_permission in allowed_permissions]):
            raise AuthorisationError("Not enough permissions to access endpoint")
    elif (not domain and dataset) or (domain and not dataset):
        raise AuthorisationError("Not enough permissions to access endpoint")


def generate_acceptable_scopes(endpoint_actions: List[str], domain: str = None, dataset: str = None) -> AcceptedScopes:
    ...
    for action in endpoint_actions:
        ...
        acceptable_sensitivities = _get_acceptable_sensitivities(dataset, domain)

        optional_scopes.add(f"{action.value}_ALL")
        for acceptable_sensitivity in acceptable_sensitivities:
            optional_scopes.add(f"{action.value}_{acceptable_sensitivity.value}")

    return AcceptedScopes(required_scopes, optional_scopes)


def _get_acceptable_sensitivities(dataset, domain):
    ...
    implied_sensitivity_map = {
        ...
        SensitivityLevel.PUBLIC: [SensitivityLevel.SENSITIVE, SensitivityLevel.PRIVATE, SensitivityLevel.PUBLIC],
        SensitivityLevel.PRIVATE: [SensitivityLevel.SENSITIVE, SensitivityLevel.PRIVATE]
    }
    acceptable_sensitivities = implied_sensitivity_map.get(sensitivity, [sensitivity]) if sensitivity else []
    return acceptable_sensitivities
