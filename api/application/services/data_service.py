...
class DataService:
    ...
    def list_raw_files(self, domain: str, dataset: str) -> list[str]:
        ...
        if len(raw_files) == 0:
            raise UserError(f"There are no uploaded files for the domain [{domain}] or dataset [{dataset}]")
        else:
            return raw_files

    def upload_dataset(self, domain: str, dataset: str, filename: str, file_contents: str) -> str:
        schema = self._get_schema(domain, dataset)
        if not schema:
            raise SchemaNotFoundError(f"Could not find schema related to the dataset [{dataset}]")
        else:
            ...
            self.persistence_adapter.upload_raw_data(schema.get_domain(), schema.get_dataset(), filename_timestamp, file_contents)
    ...
    def upload_schema(self, schema: Schema) -> str:
        if self._get_schema(schema.get_domain(), schema.get_dataset()) is not None:
            AppLogger.warning("Schema already exists for domain=%s and dataset=%s",
    ...
    def get_dataset_info(self, domain: str, dataset: str) -> EnrichedSchema:
        schema = self._get_schema(domain, dataset)
        if not schema:
            raise SchemaNotFoundError(f"Could not find schema related to the domain [{domain}] and dataset [{dataset}]")
        statistics_dataframe = self.query_adapter.query(domain, dataset, self._build_query(schema))
        last_updated = self.glue_adapter.get_table_last_updated_date(StorageMetaData(domain, dataset).glue_table_name())
    ...
    def _upload_data(self, schema: Schema, validated_dataframe: pd.DataFrame, filename: str):
    ...
    def _build_query(self, schema: Schema) -> SQLQuery:
        columns_to_query = ["count(*) as data_size", *schema.get_statistics_query_columns()]
        return SQLQuery(select_columns=columns_to_query)

    def _enrich_metadata(self,
                         schema: Schema,
                         statistics_dataframe: pd.DataFrame,
                         last_updated: str) -> EnrichedSchemaMetadata:
    ...
    def _build_date_statistics(self, enriched_columns, schema, statistics_dataframe):
        for column in schema.get_columns_by_type('date'):
            enriched_columns.append(EnrichedColumn(**column.dict(), statistics={
