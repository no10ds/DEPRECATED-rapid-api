...
class DataService:
    ...
    def upload_dataset(
        self, subject_id: str, domain: str, dataset: str, version: Optional[int], file_path: Path
    ) -> Tuple[str, int, str]:
        ...
        if not schema:
            ...
            upload_job = self.job_service.create_upload_job(
                subject_id, file_path.name, raw_file_identifier, domain, dataset, version
            )
    ...
    def query_large_data(
        self, subject_id: str, domain: str, dataset: str, version: Optional[int], query: SQLQuery
    ) -> str:
        version = handle_version_retrieval(domain, dataset, version)
        query_job = self.job_service.create_query_job(subject_id, domain, dataset, version)
