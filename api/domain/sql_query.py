...
class SQLQuery(BaseModel):
    ...
    def to_sql(self, table_name: str) -> str:
        ...
        constructed_sql = f"{select}{filter}{group_by}{aggregation_conditions}{order_by}{limit}"
    ...
    def _generate_aggregation_conditions(self):
        if self.aggregation_conditions is not None and self.aggregation_conditions != "":
    ...
    def _generate_columns(self, column_list, prefix):
        if column_list is None or len(column_list) == 0:
            return ""
        columns = ",".join([column for column in column_list if column != "" and column is not None])
