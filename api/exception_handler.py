...
from api.application.services.authorisation_service import UserCredentialsUnavailableError
...
def add_exception_handlers(app: FastAPI) -> None:
    ...
    async def general_handler(request, exc):
        ...
        return JSONResponse(content={'details': "Something went wrong. Contact your administrator."},
                            status_code=500)
    ...
    async def base_app_handler(request, exc):
        return JSONResponse(content={'details': exc.message}, status_code=exc.status_code)
    ...
    async def pydantic_error_handler(request, exc):
        return JSONResponse(content={"details": _generate_pydantic_error_message(exc.json())}, status_code=400)
