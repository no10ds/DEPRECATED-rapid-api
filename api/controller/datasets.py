...
from api.application.services.authorisation_service import protect_dataset_endpoint, protect_endpoint
from api.application.services.data_service import DataService
from api.application.services.delete_service import DeleteService
from api.application.services.format_service import FormatService
from api.common.config.auth import Action
from api.common.custom_exceptions import CrawlerStartFailsError, SchemaNotFoundError, \
    CrawlerIsNotReadyError, GetCrawlerError, AWSServiceError, UserError
...
@datasets_router.get("/{domain}/{dataset}/info",
...
@datasets_router.get("/{domain}/{dataset}/files",
                     dependencies=[Security(protect_dataset_endpoint, scopes=[Action.READ.value])])
...
@datasets_router.delete("/{domain}/{dataset}/{filename}",
                        dependencies=[Security(protect_dataset_endpoint, scopes=[Action.DELETE.value])])
async def delete_data_file(domain: str, dataset: str, filename: str, response: Response):
    try:
        ...
        raise UserError(message="Unable to delete file. Please try again later.", status_code=429)
...
@datasets_router.post("/{domain}/{dataset}", status_code=http_status.HTTP_201_CREATED,
                      dependencies=[Security(protect_dataset_endpoint, scopes=[Action.WRITE.value])])
async def upload_data(domain: str, dataset: str, response: Response, file: UploadFile = File(...)):
    try:
        file_contents = await file.read()
        filename = data_service.upload_dataset(domain, dataset, file.filename, file_contents)
        ...
        raise UserError(message="Data is currently processing. Please try again later.", status_code=429)
...
@datasets_router.post("/{domain}/{dataset}/query",
                      dependencies=[Security(protect_dataset_endpoint, scopes=[Action.READ.value])],
