...
class TestSQLQuery:
    ...
    def test_only_select_columns_provided(self):
        ...
        assert sql_query.to_sql("test_domain") == "SELECT col1,col2,col3 FROM test_domain"

    def test_only_filters_provided(self):
        sql_query = SQLQuery(filter="col1 > 16")
        assert sql_query.to_sql("test_domain") == "SELECT * FROM test_domain WHERE col1 > 16"

    def test_only_order_by_columns_provided(self):
        sql_query = SQLQuery(order_by_columns=[
        ...
        assert sql_query.to_sql("test_domain") == "SELECT * FROM test_domain ORDER BY col1 ASC,col2 ASC,col3 DESC"

    def test_only_group_by_columns_provided(self):
        sql_query = SQLQuery(group_by_columns=["col4", "col5"])
        assert sql_query.to_sql("test_domain") == "SELECT * FROM test_domain GROUP BY col4,col5"
    ...
    @pytest.mark.parametrize(
        "select_columns,filter,group_by_columns,aggregation_conditions,order_by_columns,limit,expected_sql", [
            (
                    ...
                    [SQLQueryOrderBy(column="col1"), SQLQueryOrderBy(column="col2", direction="DESC")],
                    "10",
