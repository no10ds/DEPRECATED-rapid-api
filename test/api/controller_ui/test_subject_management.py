...
class TestCreateSubjectSuccessPage(BaseClientTest):
    @patch.object(SubjectService, "get_subject_by_id")
    @patch.object(PermissionsService, "get_user_permissions_ui")
    @patch.object(Jinja2Templates, "TemplateResponse")
    def test_calls_templating_engine_with_expected_user_arguments(
        self,
        mock_templates_response,
        mock_get_user_ui_permissions,
        mock_get_subject_by_id,
    ):
        subject_template_filename = "success.html"

        mock_get_subject_by_id.return_value = UserResponse(
        ...
        response = self.client.get(
            "/subject/create/FAKE_ID123/success", cookies={"rat": "user_token"}
        )

        mock_get_subject_by_id.assert_called_once_with("FAKE_ID123")
        mock_get_user_ui_permissions.assert_called_once_with("FAKE_ID123")
        mock_templates_response.assert_called_once_with(
            name=subject_template_filename,
            context={
                "request": ANY,
    ...
    @patch.object(PermissionsService, "get_user_permissions_ui")
    @patch.object(Jinja2Templates, "TemplateResponse")
    def test_calls_templating_engine_with_expected_client_arguments(
        self,
        mock_templates_response,
        mock_get_user_ui_permissions,
        mock_get_subject_by_id,
        ...
        mock_get_subject_by_id.return_value = ClientResponse(
            client_id="FAKECLIENT_ID123",
        ...
        response = self.client.get(
            "/subject/create/FAKECLIENT_ID123/success", cookies={"rat": "user_token"}
        )

        mock_get_subject_by_id.assert_called_once_with("FAKECLIENT_ID123")
        mock_get_user_ui_permissions.assert_called_once_with("FAKECLIENT_ID123")
        mock_templates_response.assert_called_once_with(
            name=subject_template_filename,
            context={
                "request": ANY,
                "subject_id": "FAKECLIENT_ID123",
                "subject_name": "the_client_name_1",
                "secret": "some_secret",  # pragma: allowlist secret
                "subject_permissions_display_names": [
                    "Write protected test",
                    "Read private",
                ],
    ...
    def test_calls_templating_engine_with_expected_arguments_when_getting_subject_name_fails(
        ...
        mock_subject_service.get_subject_by_id.side_effect = AWSServiceError("")

        response = self.client.get(
            "/subject/create/a1b2c3d4/success", cookies={"rat": "user_token"}
        )
