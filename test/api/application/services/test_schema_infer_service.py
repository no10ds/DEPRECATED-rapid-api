...
class TestSchemaInfer:
    ...
    def test_infer_schema(self):
        expected_schema = Schema(
            ...
            columns=[
                ...
                Column(name="col_name_3", partition_index=None, data_type="Int64", allow_null=True, format=None),
                Column(name="colname_4", partition_index=None, data_type="boolean", allow_null=True, format=None)
            ]
        )
        file_content = b"colname1,colname2,Col name 3,Col/name 4! \nsomething,123,1,True\notherthing,123,3,False\n\n"

        actual_schema = self.infer_schema_service.infer_schema("mydomain", "mydataset", "PUBLIC", file_content)
    ...
    def test_raises_error_when_parsing_provided_file_fails(self, mock_pd):
        ...
        with pytest.raises(UserError):
            self.infer_schema_service.infer_schema("mydomain", "mydataset", "PUBLIC", file_content)
