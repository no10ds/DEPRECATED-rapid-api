...
from api.common.custom_exceptions import SchemaNotFoundError, CrawlerIsNotReadyError, GetCrawlerError, \
    CrawlerStartFailsError, SchemaError, ConflictError, UserError
from api.domain.enriched_schema import EnrichedSchema, EnrichedSchemaMetadata, EnrichedColumn
...
class TestUploadSchema:
    def setup_method(self):
        ...
        self.data_service = DataService(self.s3_adapter, self.crawler_adapter, self.query_adapter)
    ...
    def test_upload_schema(self):
        ...
        self.s3_adapter.save_schema.assert_called_once_with("some", "other", "PUBLIC", self.valid_schema)
...
class TestUploadDataset:
    def setup_method(self):
        ...
        self.data_service = DataService(self.s3_adapter, self.glue_adapter, self.query_adapter, self.filename_with_timestamp)
        self.valid_schema = Schema(
            metadata=SchemaMetadata(domain="some", dataset="other", sensitivity="PUBLIC",
                                    owners=[Owner(name="owner", email="owner@email.com")]),
            columns=[
    ...
    def test_upload_dataset_in_one_not_partitioned_file(self, mock_partitioner):
        ...
        filename = self.data_service.upload_dataset("some", "other", "data.csv", file_contents)
        assert filename == "2022-03-03T12:00:00-data.csv"

        self.s3_adapter.upload_partitioned_data.assert_called_once_with(
            "some",
            "other",
            "2022-03-03T12:00:00-data.csv",
            partitioned_data
        )

        self.glue_adapter.update_catalog_table_config.assert_called_once_with("some", "other")
    ...
    # E2E flow ---------------------------------------
    @patch("api.application.services.data_service.generate_partitioned_data")
    def test_upload_formatted_dataset_after_transformation_and_validation(self, mock_partitioner):
        file_contents = set_encoded_content(
    ...
    def test_raises_error_when_schema_does_not_exist(self):
        ...
        self.s3_adapter.find_schema.assert_called_once_with(
            "some",
            "other"
        )
    ...
    def test_upload_dataset_fails_when_unable_to_get_crawler_state(self):
        ...
        self.glue_adapter.check_crawler_is_ready.assert_called_once_with("some", "other")
    ...
    def test_upload_dataset_fails_when_crawler_is_not_ready(self):
        ...
        self.glue_adapter.check_crawler_is_ready.side_effect = CrawlerIsNotReadyError("msg")
    ...
    def test_upload_dataset_fails_to_start_crawler(self):
        ...
        self.glue_adapter.start_crawler.side_effect = CrawlerStartFailsError("Some thing")
...
class TestDatasetInfoRetrieval:
    def setup_method(self):
        ...
        self.data_service = DataService(self.s3_adapter, self.glue_adapter, self.query_adapter)
        self.glue_adapter.get_table_last_updated_date.return_value = "2022-03-01 11:03:49+00:00"

    def test_get_schema_information(self):
        expected_schema = EnrichedSchema(
            metadata=EnrichedSchemaMetadata(
                domain="some", dataset="other", sensitivity="PUBLIC",
                owners=[Owner(name="owner", email="owner@email.com")],
                number_of_rows=48718, number_of_columns=3,
        ...
        self.query_adapter.query.assert_called_once_with("some", "other", SQLQuery(
            select_columns=[
                "count(*) as data_size",
    ...
    def test_get_schema_size_for_datasets_with_no_dates(self):
        ...
        expected_schema = EnrichedSchema(
            metadata=EnrichedSchemaMetadata(
                domain="some", dataset="other", sensitivity="PUBLIC",
                owners=[Owner(name="owner", email="owner@email.com")],
                number_of_rows=48718, number_of_columns=1,
                last_updated="2022-03-01 11:03:49+00:00"
            ),
            columns=[
                EnrichedColumn(
