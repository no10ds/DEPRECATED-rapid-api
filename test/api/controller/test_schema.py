...
from api.common.custom_exceptions import SchemaError, ConflictError, CrawlerCreateFailsError, \
...
class TestSchemaUpload(BaseClientTest):
    ...
    def test_calls_services_successfully(self,
                                         mock_upload_schema,
                                         mock_create_crawler,
        ...
        response = self.client.post("/schema", json=request_body, headers={"Authorization": "Bearer test-token"})

        mock_upload_schema.assert_called_once_with(expected_schema)
        mock_create_user_groups.assert_called_once_with("some", "thing")
        mock_create_crawler.assert_called_once_with("some", "thing",
    ...
    def test_return_400_pydantic_error(self):
        ...
        assert response.json() == {"details": ["metadata: domain -> field required",
                                               "metadata: dataset -> field required",
                                               "metadata: sensitivity -> field required"]}

    @patch.object(GlueAdapter, "create_crawler")
    @patch.object(DataService, "upload_schema")
    def test_returns_409_when_schema_already_exists(self, mock_upload_schema, mock_create_crawler):
        request_body, expected_schema = self._generate_schema()
    ...
    @patch.object(GlueAdapter, "create_crawler")
    @patch.object(DataService, "upload_schema")
    def test_returns_400_when_invalid_schema(self, mock_upload_schema, mock_create_crawler):
        request_body, expected_schema = self._generate_schema()
    ...
    @patch.object(GlueAdapter, "create_crawler")
    @patch.object(DataService, "upload_schema")
    def test_returns_500_schema_deletion_if_crawler_creation_fails(self,
                                                                   mock_upload_schema,
                                                                   mock_create_crawler,
                                                                   mock_delete_schema,
                                                                   mock_create_user_groups,
        ...
        mock_create_crawler.side_effect = CrawlerCreateFailsError("Crawler creation error")
    ...
    @patch.object(DeleteService, "delete_schema")
    @patch.object(DataService, "upload_schema")
    def test_returns_500_schema_deletion_if_user_group_creation_fails(self,
                                                                      mock_upload_schema,
                                                                      mock_delete_schema,
                                                                      mock_create_user_groups):
        request_body, _ = self._generate_schema()
        ...
        mock_upload_schema.return_value = "some-thing.json"
        mock_create_user_groups.side_effect = UserGroupCreationError("User group creation error")
    ...
    def _generate_schema(self) -> Tuple[Dict, Schema]:
        request_body = {
            "metadata": {"domain": "some", "dataset": "thing", "sensitivity": "PUBLIC",
        ...
        expected_schema = Schema(
            metadata=SchemaMetadata(domain="some", dataset="thing", sensitivity="PUBLIC",
                                    key_value_tags={"tag1": "value1", "tag2": "value2"},
                                    key_only_tags=["tag3", "tag4"],
                                    owners=[Owner(name="owner", email="owner@email.com")]),

            columns=[
                Column(name="colname1", partition_index=None, data_type="number", allow_null=True),
                Column(name="colname2", partition_index=0, data_type="str", allow_null=False)
            ]
        )
...
class TestSchemaGeneration(BaseClientTest):
    ...
    def test_returns_schema_from_a_csv_file(self, mock_infer_schema):
        ...
        mock_infer_schema.assert_called_once_with("mydomain", "mydataset", "PUBLIC", file_content)
