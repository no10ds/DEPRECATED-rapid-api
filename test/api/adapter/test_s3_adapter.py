...
from api.common.custom_exceptions import (
    UserError, AWSServiceError,
)
from api.domain.schema import Schema, SchemaMetadata, Owner, Column
from test.test_utils import set_encoded_content, mock_schema_response, mock_list_schemas_response
...
class TestS3AdapterUpload:
    ...
    def test_store_data_throws_exception_when_file_name_is_not_provided(self):
        self._assert_store_data_raises(
            exception=UserError, message="File path is invalid", filename=None, object_content="something"
        )

    def test_store_data_throws_exception_when_file_name_is_empty(self):
        self._assert_store_data_raises(
            exception=UserError, message="File path is invalid", filename="", object_content="something"
        )
    ...
    def test_upload_partitioned_data(self):
        ...
        filename = "data.csv"
        partitioned_data = [
            ("year=2020/month=1", pd.DataFrame({
                "colname2": ["user1"]
            })),
            ("year=2020/month=2", pd.DataFrame({
                "colname2": ["user2"]
            }))
        ]
    ...
    def test_schema_upload(self):
        ...
        result = self.persistence_adapter.save_schema(domain="test_domain", dataset="test_dataset",
                                                      sensitivity="PUBLIC", schema=valid_schema)
    ...
    def test_raw_data_upload(self):
        ...
        self.persistence_adapter.upload_raw_data(domain="some", dataset="values", filename="filename.csv",
...
class TestS3AdapterDataRetrieval:
    ...
    def test_find_raw_file_when_file_exists(self):
        ...
        self.mock_s3_client.get_object.assert_called_once_with(Bucket="dataset",
                                                               Key='raw_data/domain/dataset/filename.csv')
    ...
    def test_throws_error_for_find_raw_file_when_file__does_not_exist(self):
        ...
        self.mock_s3_client.get_object.assert_called_once_with(Bucket="dataset",
                                                               Key='raw_data/domain/dataset/bad_file')
...
class TestS3Deletion:
    ...
    def test_deletion_of_raw_files_with_no_partitions(self):
        ...
        self.persistence_adapter.delete_dataset_files("domain", "dataset", '2022-03-10T12:00:00-test_file.csv')
        self.mock_s3_client.list_objects.assert_called_once_with(Bucket="data-bucket", Prefix="data/domain/dataset")
    ...
    def test_deletion_of_raw_files_when_error_is_thrown(self):
        ...
        self.mock_s3_client.delete_objects.return_value = {'Errors': [
            {
                'Key': 'An error',
...
class TestDatasetMetadataRetrieval:
    ...
    @pytest.mark.parametrize("domain, dataset, sensitivity, expected", [
    ...
    def test_retrieves_dataset_sensitivity(self,
                                           domain: str,
                                           dataset: str,
                                           sensitivity: str,
                                           expected: SensitivityLevel):
        self.mock_s3_client.list_objects.return_value = mock_list_schemas_response(domain, dataset, sensitivity)
    ...
    @pytest.mark.parametrize("domain, dataset", [
        (None, None),
        ("domain", None),
        (None, "dataset"),
    ])
    def test_returns_none_when_either_domain_or_dataset_or_both_not_specified(self, domain: str, dataset: str):
        result = self.persistence_adapter.get_dataset_sensitivity(domain, dataset)
